...
@bpm_process_registry_name :bpm_process_registry
  ...
  """
  def find_or_create_process(process_id) when is_integer(process_id) do
    if bpm_process_process_exists?(process_id) do
      {:ok, process_id}
      else
      process_id |> create_bpm_process_process
      end
  end
...
"""
def create_bpm_process_process(process_id) when is_integer(process_id) do
  case Supervisor.start_child(__MODULE__, [process_id]) do
